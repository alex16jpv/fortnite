...
const ItemDetailModal = ({ item, isOpen, onClose }: ItemDetailModalProps) => {
  ...
  const getRarityGradient = (rarity: string) => {
    ...
    return (
      rarityColors[normalizedRarity as keyof typeof rarityColors] ||
      rarityColors.common
    );
  };
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mb-4">
                  <span
                    className={`px-4 py-2 rounded-full text-sm font-semibold text-white bg-gradient-to-r ${getRarityGradient(
                      itemDetail?.rarity?.displayValue ||
                        item?.rarity?.name ||
                        "common"
                    )}`}
                  >
              ...
              <div className="grid grid-cols-1 gap-4 mb-6">
                {(itemDetail?.releaseDate || item?.firstReleaseDate) && (
                  <div className="p-4 bg-gradient-to-r from-blue-900/30 to-purple-900/30 rounded-xl border border-blue-500/20">
                    <h4 className="text-blue-300 text-sm mb-1">
                      🎯 Release Date
                    </h4>
                    <p className="text-white font-medium">
                      {itemDetail?.releaseDate
                        ? new Date(itemDetail.releaseDate).toLocaleDateString(
                            "en-US",
                            {
                              year: "numeric",
                              month: "long",
                        ...
                        : new Date(item.firstReleaseDate).toLocaleDateString(
                            "en-US",
                            {
                              year: "numeric",
                              month: "long",
              ...
              {/* Gameplay Tags */}
              {itemDetail?.gameplayTags &&
                itemDetail.gameplayTags.length > 0 && (
                  <div>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-white font-semibold">
                      🛒 Shop History ({itemDetail.shopHistory.length}{" "}
                      appearances)
                    </h3>
                  </div>
                  ...
                  <div className="bg-gray-800/30 rounded-xl p-4">
                    <div className="grid grid-cols-1 gap-3 max-h-48 overflow-y-auto">
                      {itemDetail.shopHistory
                        .slice(0, 10)
                        .map((date, index) => {
                          const appearanceDate = new Date(date);
                          const now = new Date();
                          const daysDiff = Math.floor(
                            (now.getTime() - appearanceDate.getTime()) /
                              (1000 * 60 * 60 * 24)
                          );
                          ...
                          return (
                            <div
                              key={index}
                              className="flex justify-between items-center p-3 bg-gray-700/40 rounded-lg hover:bg-gray-700/60 transition-colors"
                            >
                              ...
                              {itemDetail?.shopHistory &&
                                itemDetail?.shopHistory?.length > 0 && (
                    ...
                    {itemDetail.shopHistory.length > 10 && (
                      <div className="text-center mt-3 pt-3 border-t border-gray-700/50">
                        <span className="text-gray-400 text-sm">
                          ... and {itemDetail.shopHistory.length - 10} more
                          appearances
                        </span>
                      </div>
                    )}
                    ...
                    <div className="mt-4 pt-3 border-t border-gray-700/50">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="text-white">
                          {new Date(
                            itemDetail.shopHistory[
                              itemDetail.shopHistory.length - 1
                            ]
                          ).toLocaleDateString()}
                        </span>
                      </div>
